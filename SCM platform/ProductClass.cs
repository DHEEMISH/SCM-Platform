        ...
        public  int sumproduct(Dictionary<string, int> quantity)
        {
            ...
            //{
                ...
                if (quantity[keyOfIndex3] > 0 && quantity[keyOfIndex4] > 0)
                {
                    ...
                    {
                        sumCD = quantity[keyOfIndex4] * 30 + (quantity[keyOfIndex3] - quantity[keyOfIndex4]) * 20;
                        sumCD = quantity[keyOfIndex4] * 30 + (quantity[keyOfIndex3] - quantity[keyOfIndex4]) * 20;
                    }
                    else if (quantity[keyOfIndex4] < quantity[keyOfIndex3])
                    {
                        sumCD = quantity[keyOfIndex4] * 30 + (quantity[keyOfIndex4] - quantity[keyOfIndex3]) * 15;
                    }
                    else
                    {
                ...
                {
                    sumCD = quantity[keyOfIndex3] * 20 + quantity[keyOfIndex4] * 15;
                }
